// Supabase Edge Function: send-testimonial-invite
// Admin-invoked. Sends a testimonial/story invitation to the buyer after finalization.
...
import { testimonialInvitation } from "../_shared/email/templates.ts";
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const { data: agreement, error: loadErr } = await admin
    ...
    .select("buyer_name, buyer_email, puppy_name, agreement_status")
  ...
  if (agreement.agreement_status !== "admin_approved") {
    return jsonResponse(400, {
      error:
        "Testimonial invite can only be sent for finalized (admin_approved) agreements.",
    });
  }
  ...
  // --- Send invitation ---
  const tpl = testimonialInvitation({
    buyerName: agreement.buyer_name,
    puppyName: agreement.puppy_name,
    reviewPageUrl: `${SITE_URL}/dreamy-reviews`,
  });
  ...
  const r = await sendEmail({
    ...
    summary: `Buyer emailed testimonial invitation — ${agreement.puppy_name}`,
  });

  if (!r.ok) {
    return jsonResponse(502, {
      error: "Failed to send testimonial invitation",
      details: r.error,
    });
  }
