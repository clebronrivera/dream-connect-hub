  ...
  const { data, error } = await supabase
    ...
    .select(
      `id, name, gender, breed, color, date_of_birth, photos, primary_photo,
       ...
       is_publicly_visible, vaccinated_at, generation, personality_blurb,
...
async function updatePuppy(
  ...
  const { data, error } = await supabase
    ...
    .update(patch)
    .eq("id", p.puppyId)
    .select("id, name, gender, breed, color, date_of_birth, photos, primary_photo, video_path, description, ready_date, base_price, status, is_publicly_visible, vaccinated_at, generation, personality_blurb")
    .single();
  if (error || !data)
