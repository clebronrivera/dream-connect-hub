-- ============================================================================
-- ADMIN INSIGHTS — ADMIN-ONLY GATE (security fix)
-- Migration: 20260513000000_admin_insights_admin_gate.sql
-- ============================================================================
-- The 5 v_admin_* views from 20260512000000 claimed to be "admin-only via
-- base-table RLS" but the puppies table allows anon SELECT on
-- is_publicly_visible rows. That meant anon queries could read aggregate
-- inventory numbers (available_inventory_count, available_inventory_value,
-- per-month forward projections from listed puppies, per-breed available
-- counts) — data that's individually public via /puppies but should not be
-- exposed as a single business-intelligence aggregate.
--
-- Fix: every view's outer SELECT now gates on is_admin(). Non-admins get
-- zero rows. Financial fields (revenue, pipeline) were already protected by
-- deposit_agreements / deposit_requests / final_sales RLS, but we add the
-- gate to all views uniformly so the contract is consistent.
--
-- Trade-off vs. an alternative SECURITY DEFINER design: keeping
-- security_invoker preserves the safety property that the views can never
-- expose more than the caller could read directly. is_admin() is itself
-- SECURITY DEFINER and STABLE, so it's safe to call from an invoker view.
-- ============================================================================


-- ---------------------------------------------------------------------------
-- v_admin_kpi_summary — gated
-- ---------------------------------------------------------------------------
...
COMMENT ON VIEW public.v_admin_kpi_summary IS
  'Headline numbers for /admin Dashboard tiles. Returns one row to admins; zero rows to anyone else (is_admin() gate).';


-- ---------------------------------------------------------------------------
-- v_admin_revenue_monthly — gated
-- ---------------------------------------------------------------------------
...
COMMENT ON VIEW public.v_admin_revenue_monthly IS
  'Monthly revenue (last 24 months) split into deposits + balances collected. Admins only (is_admin() gate).';


-- ---------------------------------------------------------------------------
-- v_admin_pipeline_funnel — gated
-- ---------------------------------------------------------------------------
...
SELECT * FROM (
  ...
  UNION ALL
  SELECT 2, 'request_accepted', 'Accepted, link not yet sent',
         (SELECT count(*)::int FROM public.deposit_requests
  ...
  UNION ALL
  SELECT 3, 'request_link_sent', 'Deposit link sent, awaiting buyer',
         (SELECT count(*)::int FROM public.deposit_requests
  ...
  UNION ALL
  SELECT 4, 'agreement_sent_unpaid', 'Agreement signed, deposit unpaid',
         ...
         (SELECT coalesce(sum(purchase_price), 0)::numeric
           ...
           WHERE agreement_status = 'sent' AND deposit_status = 'pending')
  UNION ALL
  SELECT 5, 'agreement_deposit_confirmed', 'Deposit confirmed, awaiting admin approval',
         (SELECT count(*)::int FROM public.deposit_agreements
         ...
         (SELECT coalesce(sum(purchase_price), 0)::numeric
           ...
           WHERE agreement_status = 'sent' AND deposit_status = 'admin_confirmed')
  UNION ALL
  SELECT 6, 'agreement_admin_approved', 'Admin-approved, awaiting balance',
         (SELECT count(*)::int FROM public.deposit_agreements
         ...
         (SELECT coalesce(sum(purchase_price), 0)::numeric
           ...
           WHERE agreement_status = 'admin_approved')
  UNION ALL
  SELECT 7, 'agreement_complete', 'Sale complete',
         (SELECT count(*)::int FROM public.deposit_agreements
...
COMMENT ON VIEW public.v_admin_pipeline_funnel IS
  'Pipeline funnel: count + $ at each conversion stage. Admins only (is_admin() gate).';


-- ---------------------------------------------------------------------------
-- v_admin_breed_performance — gated
-- ---------------------------------------------------------------------------
...
COMMENT ON VIEW public.v_admin_breed_performance IS
  'Per-breed: inventory, inquiries, completed-sale revenue, conversion. Admins only (is_admin() gate).';


-- ---------------------------------------------------------------------------
-- v_admin_forward_projection — gated
-- ---------------------------------------------------------------------------
...
COMMENT ON VIEW public.v_admin_forward_projection IS
  'Forward revenue projection by month (next 18 months). Admins only (is_admin() gate).';
